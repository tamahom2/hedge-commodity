...
public class Put extends Strategy {

	
	private double swapRate;
	
	public Put(double putPercent, double swapRate) {
		super(putPercent,0.0);
		this.swapRate = swapRate;
	}
	
	public Put(double putPercent, double strike, double expiry) {
		super(putPercent, 0, strike, expiry);
	}
	
	public Put(double putPercent) {
		super(putPercent,0);
	}
	...
	public double payoff(double S, double X, double spread) {
		...
		return  this.putPercent*Math.max(X*(1-spread)-S, 0);
	}
	...
	public double premium(double S, double K, double T, double r, double b, double sigma, double spread) {
		return this.putPercent*VanillaOptions.generalBlackScholes(S, K*(1-spread), T, r, b, sigma, "p");
	}
	...
	public double simulateOneVolume(double S0, double strike, double r, int duration, Scenario scenario, double hedgedVolume,
		...
		for(int i=0;i<duration;i++) {
			...
			double currentValueOption = premium(forwardCurve.get(duration).close, strike, expiry, 0, 0, vol, 0);
			if(i==0)
				totalPremium += hedgedVolume*currentValueOption;
			System.out.println("Value of Option in month " + (i+1) + " is : " + currentValueOption + " and will expire in : " + expiry);
		}
		totalWithHedging = hedgedVolume*(price - payoff(price, strike, 0));
		System.out.println("Total Hedging : " + totalWithHedging);
		return totalWithHedging+totalPremium;
	}
